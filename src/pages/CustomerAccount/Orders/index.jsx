...
import { Box, Tabs, Tab, Typography, Pagination, Stack, FormControl, Select, MenuItem, TextField, Button } from "@mui/material";
import "./Orders.scss";
...
function Orders() {
  ...
  useEffect(() => {
    const getData = async () => {
      ...
      setOrders([])
      await apiCart.getOrdersFilterOfUser(param)
        .then(response=>{
  ...
  const handleFilter = () => {
    ...
    const getData = async () => {
        ...
        setOrders([])
        apiCart.getOrdersFilterOfUser(param)
            .then(response=>{
            setOrders(response.data.carts);
            setMaxPage(response.data.maxPage);
            })
  ...
  return (
    <>
      ...
      <Box className="myorder" sx={{ width: "100%" }}>
        ...
        <Box style={{marginTop: "10px", backgroundColor: "aliceblue", borderRadius: "5px"}}>
          <Stack direction="row" alignItems="center" spacing={1} style={{flexWrap: "wrap", justifyContent: "space-between"}}>
            <Stack direction="column" sx={{width: "48%"}}>
              <FormControl sx={{ m: 1, minWidth: 120, maxWidth: 600, flexDirection: "row", alignItems: "center", justifyContent: "space-between" }}>
                  <Typography className={"create-selectbox__label"}>Thanh toán bằng:</Typography>
                  <Select
                      value={paymentMethod}
            ...
            <Stack direction="column" sx={{width: "45%"}}>
                <FormControl sx={{ m: 1, minWidth: 120, maxWidth: 600, flexDirection: "row", alignItems: "center", justifyContent: "space-between" }}>
                    <Typography className={"create-selectbox__label"}>Ngày tạo:</Typography>
                    <BasicDateRangePicker onChangeCreatedDate={handleChangeCreatedDate} value={createdDate} className="create-selectbox__input" status="ClientOrderFilter"/>
                </FormControl>
            </Stack>
            <Stack direction="column" sx={{width: "45%"}}>
              <FormControl sx={{ m: 1, minWidth: 120, maxWidth: 600, flexDirection: "row", alignItems: "center", justifyContent: "space-between" }}>
                  <Typography className={"create-selectbox__label"}>Tổng hóa đơn:</Typography>
            ...
            <Stack direction="column" sx={{width: "45%"}}>
                <FormControl sx={{ m: 1, minWidth: 120, maxWidth: 600, flexDirection: "row", alignItems: "center", justifyContent: "space-between" }}>
                    <Typography className={"create-selectbox__label"}>Sắp xếp loại:</Typography>
                    <Select
                        value={sortBy}
            ...
            <Stack direction="column" sx={{width: "45%"}}>
                <FormControl sx={{ m: 1, minWidth: 120, maxWidth: 600, flexDirection: "row", alignItems: "center", justifyContent: "space-between" }}>
                    <Typography className={"create-selectbox__label"}>Sắp xếp theo:</Typography>
                    <Select
                        value={order}
          ...
          <Stack direction="row" alignItems="center" spacing={1} style={{flexWrap: "wrap", justifyContent: "space-between"}}>
            <Stack width="130px">
                  <Button variant="contained" startIcon={<FilterAltIcon/>} style={{padding: "7px 10px", margin: "0px 0px 10px 10px"}} onClick={handleFilter}>Bộ lọc</Button>
              </Stack>
              <Stack width="130px">
                  <Button variant="contained" endIcon={<RotateLeftIcon/>} style={{padding: "7px 10px", margin: "0px 10px 10px 0px"}} onClick={handleReset}>Làm mới</Button>
              </Stack>
          </Stack>
        </Box>
        <Box>
          {orders.length === 0 ?
                (
                  <TabPanel key={selected} value={selected} index={selected} dir={theme.direction}>
          ...
          {maxPage > 1 ? <Stack spacing={2}>
            <Pagination count={maxPage} page={currentPage} onChange={handleChangePage} style={{justifyContent: "center", display: "flex"}}/>
          </Stack> : <></>}
        </Box>
      </Box>
    </>
